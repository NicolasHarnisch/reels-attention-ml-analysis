...
def main():
    ...
    # Definição das variáveis de hábitos para a Versão B (sem target)
    features_b = [
    ...
    # Aplicação de PCA em 2 componentes na Versão B
    ...
    # -------------------------------------------------------------------------
    # 1. VALIDAÇÃO DE K - VERSÃO B SEM PCA (Espaço 5D original)
    # -------------------------------------------------------------------------
    wcss_sem_pca = []
    sil_sem_pca = []
    ...
    # -------------------------------------------------------------------------
    # 2. VALIDAÇÃO DE K - VERSÃO B COM PCA (Espaço 2D reduzido)
    # -------------------------------------------------------------------------
    wcss_com_pca = []
    sil_com_pca = []
    ...
    # Encontrar K ótimo por silhueta para com PCA
    ...
    # -------------------------------------------------------------------------
    # 3. TREINAMENTO COM K=3 E AVALIAÇÃO DE RESULTADOS
    # -------------------------------------------------------------------------
    k_ideal = 3
    ...
    # Salvar médias de atenção para sem PCA
    ...
    df['cluster_b'] = kmeans_com.fit_predict(X_b_pca) # cluster_b oficial
    
    # Salvar médias de atenção para com PCA
    ...
    sil_a = 0.1129 # valor histórico da Versão A
    ...
    # Salvar perfis completos da versão oficial (com PCA)
    ...
    # Salvar resultados estatísticos em arquivo texto de suporte
    ...
    # Plotar Boxplot de attention_span_score por cluster B
    ...
    # Plotar gráfico dos clusters B (Visualização no Espaço PCA de alta separação)
    plt.figure(figsize=(8, 6))
    ...
    # Plotar cotovelo e silhueta específicos da Versão B com PCA
    ...
    # Salvar dataset limpo atualizado com as labels dos clusters
